...
intptr_t _mock(const char *function, const char *parameters, ...) {
    ...
    if (expectation != NULL) {
        ...
        // fix memory leak issue: free memory allocated for those exceptions that are not kept
        // in the expectation queues after the mock function was called.
...
void trigger_unfulfilled_expectations(Vector *_expectation_queue, TestReporter *reporter) {
    int i;
    for (i = 0; i < vector_size(_expectation_queue); i++) {
        RecordedExpectation *expectation = vector_get(_expectation_queue, i);
        if (! expectation->should_keep) {
...
RecordedExpectation *find_expectation(const char *function) {
    ...
    for (i = 0; i < vector_size(expectation_queue); i++) {
	    ...
	    if (strcmp(expectation->function, function) != 0) {
	        _expect_not(function,expectation->test_file,expectation->test_line);
            vector_remove(expectation_queue, i);
	       return NULL; 
        }
        else {
            if (! expectation->should_keep) {
