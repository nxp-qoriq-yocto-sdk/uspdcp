...
void _add_test(TestSuite *suite, const char *name, void (*test)()) {
    suite->size++;
...
void _add_suite(TestSuite *owner, const char *name, TestSuite *suite) {
    owner->size++;
...
static void run_the_test_code(TestSuite *suite, UnitTest *test, TestReporter *reporter) {
	...
	(*suite->setup)();
    (*test->test)();
    // free the mock resources before calling teardown function so that 
    // the memory leak detrection mechanism to not fail because of the memory allocated during the test run
    // for the expects. Because of this no expects should be used in the teardown function.
